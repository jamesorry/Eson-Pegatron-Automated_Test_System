...
#if 0
bool HMI_Command::Response_OHM_Reader(uint32_t ohm_A, uint32_t ohm_B, uint32_t ohm_C, uint32_t ohm_D)
{
	uint8_t i;
	HMICmdRec rec;
	rec.datatype = QUEUE_DATA_TYPE_INDICATION;
	rec.data[HMI_CMD_BYTE_TAGID] = ControllerTagID;
	rec.data[HMI_CMD_BYTE_LENGTH] = HMI_CMD_LEN_BASE + 16;
	rec.data[HMI_CMD_BYTE_CMDID] = HMI_CMD_RECORD_OHM_RESELT;
    rec.data[HMI_CMD_BYTE_HMIID] = maindata.HMI_ID;
//    cmd_port->println("ohm_A: " + String(ohm_A));
//    cmd_port->println("ohm_B: " + String(ohm_B));
//    cmd_port->println("ohm_C: " + String(ohm_C));
//    cmd_port->println("ohm_D: " + String(ohm_D));
	for(uint8_t i=0; i<4; i++)
		rec.data[HMI_CMD_BYTE_DATA+i] = (ohm_A >> (3-i)*8)& 0xff;
	for(uint8_t i=4; i<8; i++)
		rec.data[HMI_CMD_BYTE_DATA+i] = (ohm_B >> (7-i)*8)& 0xff;
	for(uint8_t i=8; i<12; i++)
		rec.data[HMI_CMD_BYTE_DATA+i] = (ohm_C >> (11-i)*8)& 0xff;
	for(uint8_t i=12; i<16; i++)
		rec.data[HMI_CMD_BYTE_DATA+i] = (ohm_D >> (15-i)*8)& 0xff;
	rec.data[rec.data[HMI_CMD_BYTE_LENGTH]-1] = HMI_CMD_ComputeCRC(rec.data);
	rec.datalen = rec.data[HMI_CMD_BYTE_LENGTH];
	rec.retrycnt = 0;
	cmdQueue->push(&rec);
...
#if HMI_CMD_DEBUG
	cmd_port->println("Request_OHM_Reader()");
#endif
	return true;
}

bool HMI_Command::Response_OHM_Tare()
{
	uint8_t i;
	HMICmdRec rec;
	rec.datatype = QUEUE_DATA_TYPE_INDICATION;
	rec.data[HMI_CMD_BYTE_TAGID] = ControllerTagID;
	rec.data[HMI_CMD_BYTE_LENGTH] = HMI_CMD_LEN_BASE;
	rec.data[HMI_CMD_BYTE_CMDID] = HMI_CMD_OHM_TARE;
    rec.data[HMI_CMD_BYTE_HMIID] = maindata.HMI_ID;
	rec.data[rec.data[HMI_CMD_BYTE_LENGTH]-1] = HMI_CMD_ComputeCRC(rec.data);
	rec.datalen = rec.data[HMI_CMD_BYTE_LENGTH];
	rec.retrycnt = 0;
...
#if HMI_CMD_DEBUG
	cmd_port->println("Response_OHM_Tare()");
#endif
	return true;
}

bool HMI_Command::Response_OHM_Scale()
{
	uint8_t i;
	HMICmdRec rec;
	rec.datatype = QUEUE_DATA_TYPE_INDICATION;
	rec.data[HMI_CMD_BYTE_TAGID] = ControllerTagID;
	rec.data[HMI_CMD_BYTE_LENGTH] = HMI_CMD_LEN_BASE + 16;
	rec.data[HMI_CMD_BYTE_CMDID] = HMI_CMD_OHM_SCALE;
    rec.data[HMI_CMD_BYTE_HMIID] = maindata.HMI_ID;
	rec.data[rec.data[HMI_CMD_BYTE_LENGTH]-1] = HMI_CMD_ComputeCRC(rec.data);
	rec.datalen = rec.data[HMI_CMD_BYTE_LENGTH];
	rec.retrycnt = 0;
...
#if HMI_CMD_DEBUG
	cmd_port->println("Response_OHM_Scale()");
#endif
	return true;
}

bool HMI_Command::Response_HMIID()
{
	uint8_t i;
	HMICmdRec rec;
	rec.datatype = QUEUE_DATA_TYPE_INDICATION;
	rec.data[HMI_CMD_BYTE_TAGID] = ResponseTagID;
	rec.data[HMI_CMD_BYTE_LENGTH] = HMI_CMD_LEN_BASE;
	rec.data[HMI_CMD_BYTE_CMDID] = HMI_CMD_SETID;
    rec.data[HMI_CMD_BYTE_HMIID] = maindata.HMI_ID;
	rec.data[rec.data[HMI_CMD_BYTE_LENGTH]-1] = HMI_CMD_ComputeCRC(rec.data);
	rec.datalen = rec.data[HMI_CMD_BYTE_LENGTH];
	rec.retrycnt = 0;
...
#if HMI_CMD_DEBUG
	cmd_port->println("Response_HMIID()");
#endif
	return true;
}

bool HMI_Command::Response_TareADC(uint32_t TareADC_A, uint32_t TareADC_B, uint32_t TareADC_C, uint32_t TareADC_D)
{
	uint8_t i;
	HMICmdRec rec;
	rec.datatype = QUEUE_DATA_TYPE_INDICATION;
	rec.data[HMI_CMD_BYTE_TAGID] = ControllerTagID;
	rec.data[HMI_CMD_BYTE_LENGTH] = HMI_CMD_LEN_BASE + 16;
	rec.data[HMI_CMD_BYTE_CMDID] = HMI_CMD_TARE_ADC;
    rec.data[HMI_CMD_BYTE_HMIID] = maindata.HMI_ID;
	for(uint8_t i=0; i<4; i++)
		rec.data[HMI_CMD_BYTE_DATA+i] = (TareADC_A >> (3-i)*8)& 0xff;
	for(uint8_t i=4; i<8; i++)
		rec.data[HMI_CMD_BYTE_DATA+i] = (TareADC_B >> (7-i)*8)& 0xff;
	for(uint8_t i=8; i<12; i++)
		rec.data[HMI_CMD_BYTE_DATA+i] = (TareADC_C >> (11-i)*8)& 0xff;
	for(uint8_t i=12; i<16; i++)
		rec.data[HMI_CMD_BYTE_DATA+i] = (TareADC_D >> (15-i)*8)& 0xff;
	rec.data[rec.data[HMI_CMD_BYTE_LENGTH]-1] = HMI_CMD_ComputeCRC(rec.data);
	rec.datalen = rec.data[HMI_CMD_BYTE_LENGTH];
	rec.retrycnt = 0;
	cmdQueue->push(&rec);
...
#if HMI_CMD_DEBUG
    cmd_port->println("A Tare:" + String(maindata.OFF_A));
    cmd_port->println("B Tare:" + String(maindata.OFF_B));
    cmd_port->println("C Tare:" + String(maindata.OFF_C));
    cmd_port->println("D Tare:" + String(maindata.OFF_D));
	cmd_port->println("Response_TareADC()");
#endif
	return true;
}

bool HMI_Command::Response_ScaleADC(uint32_t ScaleADC_A, uint32_t ScaleADC_B, uint32_t ScaleADC_C, uint32_t ScaleADC_D)
{
	uint8_t i;
	HMICmdRec rec;
	rec.datatype = QUEUE_DATA_TYPE_INDICATION;
	rec.data[HMI_CMD_BYTE_TAGID] = ControllerTagID;
	rec.data[HMI_CMD_BYTE_LENGTH] = HMI_CMD_LEN_BASE + 16;
	rec.data[HMI_CMD_BYTE_CMDID] = HMI_CMD_SCALE_ADC;
    rec.data[HMI_CMD_BYTE_HMIID] = maindata.HMI_ID;
	for(uint8_t i=0; i<4; i++)
		rec.data[HMI_CMD_BYTE_DATA+i] = (ScaleADC_A >> (3-i)*8)& 0xff;
	for(uint8_t i=4; i<8; i++)
		rec.data[HMI_CMD_BYTE_DATA+i] = (ScaleADC_B >> (7-i)*8)& 0xff;
	for(uint8_t i=8; i<12; i++)
		rec.data[HMI_CMD_BYTE_DATA+i] = (ScaleADC_C >> (11-i)*8)& 0xff;
	for(uint8_t i=12; i<16; i++)
		rec.data[HMI_CMD_BYTE_DATA+i] = (ScaleADC_D >> (15-i)*8)& 0xff;
	rec.data[rec.data[HMI_CMD_BYTE_LENGTH]-1] = HMI_CMD_ComputeCRC(rec.data);
	rec.datalen = rec.data[HMI_CMD_BYTE_LENGTH];
	rec.retrycnt = 0;
	cmdQueue->push(&rec);
...
#if HMI_CMD_DEBUG
    cmd_port->println("A Scale:" + String(maindata.CO_A));
    cmd_port->println("B Scale:" + String(maindata.CO_B));
    cmd_port->println("C Scale:" + String(maindata.CO_C));
    cmd_port->println("D Scale:" + String(maindata.CO_D));
	cmd_port->println("Response_ScaleADC()");
#endif
	return true;
}
#endif
