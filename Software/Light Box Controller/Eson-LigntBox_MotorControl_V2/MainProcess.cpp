...
bool MotorGoHome(int motornum)
{
	...
	{
		case 0:
			if(motornum == 0) MotorServoSearchSensor();
			else if(motornum == 1) MotorVRSearchSensor();
...
void MotorServoSearchSensor(int pin, int dir, int toggletimes)
{
	...
	if(dir < 0)
		Motor[0]->Speed(-SPEED_SERVO_SEARCH);
	else
		Motor[0]->Speed(SPEED_SERVO_SEARCH);
}
...
void MotorVRSearchSensor(int dir, int toggletimes)
{
	...
	if(dir < 0)
		Motor[1]->Speed(-SPEED_SERVO_SEARCH);
	else
		Motor[1]->Speed(SPEED_SERVO_SEARCH);
}
...
void goHome(int motornum)
{
	if(motornum == 0)
	{
		...
		Motor[motornum]->Speed(SPEED_SERVO_GOHOME);
	}
    else if(motornum == 1)
    {
        Motor[motornum]->MoveTo(maindata.VR_HomeOffset, 1000);
    }
}
	...
	switch(runtimedata.Workindex[WORKINDEX_SEARCH_SENSOR])
	{
		case 0:
            ...
            if(maindata.TargetStation - runtimedata.Station > 0){
                ...
                runtimedata.SerarchDir = MOTOR_CW;//正轉
            }
            else{
                runtimedata.SerarchPin = maindata.TargetStation + 1;
