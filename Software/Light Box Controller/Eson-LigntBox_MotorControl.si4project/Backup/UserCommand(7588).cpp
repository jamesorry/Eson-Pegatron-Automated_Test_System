...
void cmdSearchSensor()
{
	...
	if(getNextArg(arg2))
	    dir = arg2.toInt();
    if(getNextArg(arg3))
        toggletime = arg3.toInt();
    cmd_port->println("pin:" + String(pin));
	...
	runtimedata.SerarchPin = pin;
	SearchSensor(pin, dir, toggletime);
}
...
void cmdOffestStopPin()
{
    ...
    if(arg1.length()==0)
    {
        cmd_port->println("offset: " + String(maindata.OffsetDistanceOfStopPin));
        return;
    }
	offset = arg1.toInt();
