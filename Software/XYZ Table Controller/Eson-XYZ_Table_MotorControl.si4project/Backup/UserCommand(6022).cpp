...
void cmdHMIID(void)
{
	...
	if (getNextArg(arg1))
	{
		maindata.HMI_ID = arg1.toInt();
		runtimedata.UpdateEEPROM = true;
	}
	cmd_port->println("HMI ID: 0x" + String(maindata.HMI_ID, HEX));
}
...
void cmdBTCtrl()
{
	...
	{
		//	發送 BLE AT 指令
		digitalWrite(BT_PWRC, LOW);  //  設定為LOW  發BLE 藍芽AT指令 HIGH 正常BLE  
		delay(300);

		for (int i = 0; i < 10; i++)	
		{
			BT_PORT.println("AT+VERSION");	 // Wake BLE 4.2 模塊
			delay(500);   
		}  

		BT_PORT.println("AT+DEFAULT");
		delay(300); 
		BT_PORT.println("AT+STARTEN1");
		delay(300); 
		BT_PORT.println("AT+ALED1");
		delay(300); 	
		BT_PORT.print("AT+NAME");
		BT_PORT.println(arg1);
		delay(300); 
		BT_PORT.println("AT+RESET");
		//  delay(500); 
		digitalWrite(BT_PWRC, HIGH);  //  設定為LOW	發BLE 藍芽AT指令 HIGH 正常BLE   
